...
def setEnv(pipelineParams) {
	...
	env.NEXUS_URL = pipelineParams.nexusUrl;
...
def checkout() {
	...
	stage('Checkout') {
		...
		if(BRANCH_NAME ==~ /(release.*)/) {
			...
			sh (
					script: "mvn -f ${pipelineParams.project_folder}/pom.xml -B help:evaluate -Dexpression=project.version | grep -e '^[^\\[]' > release.txt",
					returnStdout: true,
					returnStatus: false
					)
